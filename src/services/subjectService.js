...
const getSubjectsByCareer = async (career_id) => {
  try {
    ...
    const subjects = await Subject.find({ career_id: career_id });
...
const getPrerequisites = async (subject_id) => {
  ...
  const prerequisites = await SubjectRelationship.find({ subject_id: subject_id });
...
const prerequisites_map = async (career_id) => {
  try{
      const subjects = await Subject.find({ career_id:career_id }, 'id');
      if (!subjects){
          throw new Error('No se encontraron materias para la carrera');
      }
      const subjectsObjects = [];
      for (const subject of subjects){
        const prerequisites = await SubjectRelationship.find({ subject_id: subject.id},'preSubject_id');
...
const postrequisites_map = async (career_id) => {
  try{
      ...
      const subjectsObjects = [];
      for (const subject of subjects){
        const postrequisites = await SubjectRelationship.find({ preSubject_id: subject._i});;
